...
public:
    ...
    void find_markers(cv::Mat const& im, std::map<int, polygon_t>& markers);
    void draw_frame(cv::Mat& plot, cv::Vec3f const& p, cv::Vec4f q);
    void draw_found_markers(cv::Mat& plot, std::map<int, polygon_t> const& markers);
};
