...
template <typename T>
T json_get(jsonxx::Array const& arr, int i);

template <>
inline std::string json_get<std::string>(jsonxx::Array const& arr, int i)
{
    return arr.get<jsonxx::String>(i);
}

template <>
inline double json_get<double>(jsonxx::Array const& arr, int i)
{
    return (double)arr.get<jsonxx::Number>(i);
}

template <>
inline int json_get<int>(jsonxx::Array const& arr, int i)
{
    return (int)arr.get<jsonxx::Number>(i);
}

template <>
inline float json_get<float>(jsonxx::Array const& arr, int i)
{
    return (float)arr.get<jsonxx::Number>(i);
}

template <typename ElemType, int Size>
inline std::array<ElemType, Size> json_get_array(jsonxx::Object const& obj, std::string const& name)
{
    if (!obj.has<jsonxx::Array>(name))
        throw std::runtime_error("json doesn't have array entry " + name);

    auto const& arr = obj.get<jsonxx::Array>(name);
    std::array<ElemType, Size> res;

    if (arr.size() != Size)
        throw std::runtime_error("array has incorrect size " + name);

    for (int i = 0; i < Size; ++i)
        res[i] = json_get<ElemType>(arr, i);

    return res;
}

template <typename ElemType>
inline std::vector<ElemType> json_get_vector(jsonxx::Object const& obj, std::string const& name)
{
    if (!obj.has<jsonxx::Array>(name))
        throw std::runtime_error("json doesn't have array entry " + name);

    auto const& arr = obj.get<jsonxx::Array>(name);
    std::vector<ElemType> res(arr.size());
...
inline jsonxx::Object json_load(std::string const& filename)
{
    std::ifstream f(filename.c_str());
    if (f.fail())
        throw std::runtime_error("can't find file json " + filename);
    ...
    std::string s(n, '\0');
    f.read((char*)s.data(), n);
    bool parsed = obj.parse(s);
    if (!parsed)
...
template <typename T>
inline T json_get(jsonxx::Object const& obj, std::string const& name)
{
    if (!obj.has<T>(name))
        throw std::runtime_error("json doesn't have required entry " + name);

    return obj.get<T>(name);
}

inline double json_get(jsonxx::Object const& obj, std::string const& name, double low, double up)
{
    if (!obj.has<jsonxx::Number>(name))
        throw std::runtime_error("json doesn't have required entry " + name);

    double result = obj.get<jsonxx::Number>(name);
    if (result < low || result > up)
        throw std::runtime_error("json entry " + name + " is out of range " + std::to_string(low) + ".." + std::to_string(up));
